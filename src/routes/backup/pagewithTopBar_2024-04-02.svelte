...
$: filteredSites = sites.filter(site => {
    ...
    // ✅ Ensure `site.category` matches correctly
    const categoryMatch =
        categories.length === 0 || 
        (Array.isArray(site.category) 
            ? site.category.some(cat => categories.includes(cat)) // If category is an array
            : categories.includes(site.category) // If category is a string
        );
...
// First time user pop up.

let showPopup = false;

onMount(() => {
  // Check if the user has seen the popup
  if (!localStorage.getItem("vizlabPopupSeen")) {
    showPopup = true;
  }
});
...
function closePopup() {
  showPopup = false;
  localStorage.setItem("vizlabPopupSeen", "true"); // Mark as seen
}
...
{#if showPopup}
  ...
  >
    <div 
      class="bg-white p-6 rounded-lg max-w-xl text-center relative"
      on:click|stopPropagation
    >
      <button 
        class="absolute top-2 right-4 text-gray-600 hover:text-black text-xl"
        on:click={closePopup}
      >
        ✖
      </button>

      <h2 class="text-2xl font-bold m-auto mb-4">Welcome to Vizlab.org</h2>
	  <p class="text-gray-700 m-auto">
        The best collection of resources for the 3D artist on the internet. 
      </p>
	  <h3 class="text-2xl m-auto font-bold mb-4">
		Use the filters to find what you need!
	  </h3>
      <!-- 
	  <p class="text-gray-700 m-auto">
        Vizlab.org is a collection of educational tools and resources. Use the filters and quick tabs to find what you need!
      </p>
	  <h3 class="text-2xl font-bold mb-4">Welcome to Vizlab.org</h3>
	  -->
      <button 
